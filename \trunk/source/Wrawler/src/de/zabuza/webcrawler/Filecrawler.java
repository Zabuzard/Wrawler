...
public final class Filecrawler {
	 ...
	 * @param path Path to file with external data
	...
	public static Map<Calendar, ExtEventData> processExternalEventData(String path) throws IOException {
		...
		List<String[]> contentSplit = new ArrayList<String[]>(content.size());
		...
		Map<Calendar, ExtEventData> events = new TreeMap<Calendar, ExtEventData>();
		//Process all events
		for (int i = OFFSET_EVENT; i < typeHeader.length; i++) {
			...
			if (typeText.equals("CO") || typeText.equals("COOP")) {
				type = EventType.COOP;
			} else if (typeText.equals("SPVP") || typeText.equals("PVP")
					|| typeText.equals("TVT")) {
				type = EventType.TVT;
				...
				System.err.println("External file error: Unknown event type '"
						+ typeText + "' in column " + i);
			}
	...
	public static Map<String, ExtPlayerData> processExternalPlayerData(String path) throws IOException {
		...
		Map<String, ExtPlayerData> playersData =
				new TreeMap<String, ExtPlayerData>(new InsensitiveComparator());
		...
		for (int i = OFFSET_PLAYERS; i < content.size(); i++) {
			...
			if (!registrationDateText.equals("")) {
				...
				System.err.println("External file error: Unknown registration date for player '"
						+ player + "'");
			}
			
			//Get player notification of inactivity status
			
			//Take care of index because field can be missing (then it and all following lines where empty)
			...
			ExtPlayerData playerDate = new ExtPlayerData(player, registrationDate,
					reactivationDate, inactivityNotification);
	 ...
	 * @param extEventMap Map that contains external event data
	 * @param extPlayerMap Map that contains external player data
	 */
	public static void exportToExternalFile(Map<Calendar, ExtEventData> extEventMap,
		...
		Map<String, Map<Calendar, SlotStatus>> playerEventData =
				new TreeMap<String, Map<Calendar, SlotStatus>>(new InsensitiveComparator());
		for (Entry<Calendar, ExtEventData> entry : extEventMap.entrySet()) {
			for (String player : entry.getValue().getPlayers()) {
				...
				if (UserTableDb.getInstance().getId(player) <= 0) {
					continue;
				}
				...
				playerEventData.get(player).put(entry.getKey(),
						entry.getValue().getPlayerStatus(player));
			}
		}
		
		//Build header and event lines
		List<String> result = new ArrayList<String>(playerEventData.size() + OFFSET_PLAYERS);
		...
		//Build player lines
		List<Calendar> eventDateList = new ArrayList<Calendar>(extEventMap.keySet());
		...
		for (Entry<String, Map<Calendar, SlotStatus>> entry : playerEventData.entrySet()) {
			...
			if (!extPlayerMap.containsKey(player)) {
				//TODO How to catch new players & players that don't have played yet?
				//System.err.println("Skipping for external data unknown player: " + player);
				continue;
				...
				if (extPlayerData.getReactivationDate() != null) {
					playerLine.append(CrawlerUtil.convertDateToString(
							extPlayerData.getReactivationDate()));
				}
				playerLine.append(SPLIT_VALUE);
				if (extPlayerData.getRegistrationDate() != null) {
					playerLine.append(CrawlerUtil.convertDateToString(
							extPlayerData.getRegistrationDate()));
				}
				playerLine.append(SPLIT_VALUE);
		...
		try {
			wr = new BufferedWriter(new FileWriter(FILEPATH_OUTPUT));
			...
			try {
				...
				System.err.println("Unknown error while closing external file export stream.");
				e.printStackTrace();
			}
		}
	}
