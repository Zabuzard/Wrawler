...
public final class Program {
	...
	public static void main(final String[] args) throws IOException {
		 ...
		 * list (assuming it exists).
		 * 
		 * ********************** CAUTION ********************** Crawling, of
		 * course, causes a heavy amount of web-traffic. If you use this
		 * frequently the server can interpret this as DDos! This could lead to
		 * legal consequences for the executor of this program! So use "true"
		 * only to create and serialize a list, after that only use "false"
		...
		/*
		 * //List events of player String participatingPlayerRegex = "Zabuza";
		 * List<EventData> eventsWhereParticipated =
		 * metrics.getEventsWhereParticipated(participatingPlayerRegex); for
		 * (EventData datum : eventsWhereParticipated) {
		 * System.out.println(datum.getName()); }
		 */

		/*
		 * //Get Lead ranking Map<String, Integer> leadRanking =
		 * metrics.getLeadRanking(); for (Entry<String, Integer> entry :
		 * leadRanking.entrySet()) { System.out.println(entry.getKey() + ":" +
		 * entry.getValue()); }
		 */

		/*
		 * //Get Events participated ranking Map<String, Integer>
		 * eventsParticipatedRanking = metrics.getEventsParticipatedRanking();
		 * for (Entry<String, Integer> entry :
		 * eventsParticipatedRanking.entrySet()) {
		 * System.out.println(entry.getKey() + ":" + entry.getValue()); }
		 */

		/*
		 * //Average event size int beforeMonths = -6; Calendar since =
		 * Calendar.getInstance(); since.add(Calendar.MONTH, beforeMonths);
		 * System.out.println(metrics.getAvgEventSize(since) +
		 * " : Average event size since '" + beforeMonths + " months'");
		 * //Average absent players since = Calendar.getInstance();
		 * since.add(Calendar.MONTH, beforeMonths);
		 * System.out.println(metrics.getAvgAbsentPlayers(since) +
		 * " : Average absent players since '" + beforeMonths + " months'");
		 * //Average absent or late signed out players since =
		 * Calendar.getInstance(); since.add(Calendar.MONTH, beforeMonths);
		 * System.out.println(metrics.getAvgAbsentOrLateSignoutPlayers(since) +
		 * " : Average absent or signed out late players since '" + beforeMonths
		 * + " months'"); //Average absent, late signed out or signed out
		 * players since = Calendar.getInstance(); since.add(Calendar.MONTH,
		 * beforeMonths);
		 * System.out.println(metrics.getAvgAbsentOrLateSignoutOrSigneoutPlayers
		 * (since) +
		...
		/*
		 * //Get event size ranking Map<EventData, Integer> eventSizeRanking =
		 * metrics.getEventSizeRanking(); for (Entry<EventData, Integer> entry :
		 * eventSizeRanking.entrySet()) { String eventText =
		 * CrawlerUtil.convertDateToString(entry.getKey().getDate()) + " : " +
		 * entry.getKey().getType() + " : " + entry.getKey().getName();
		 * System.out.println(entry.getValue() + " : " + eventText); }
		 */

		/*
		 * //Get events of creator String creatorRegex = "(Pfleger)";
		 ...
		 * list.exportDatabaseFormat(); System.out.println("Exporting done.");
		 * 
		 * 
		 * System.out.println("Importing from database format..."); EventList
		 * importedList = EventList.importDatabaseFormat();
		 * System.out.println("Importing done.");
		 * 
		 * 
		 * System.out.println("Exporting to extEventData..."); Map<Calendar,
		 * ExtEventData> importedExtEventData =
		 * importedList.exportToExtEventDataMap();
		 * Filecrawler.exportToExternalFile(importedExtEventData,
		 * extPlayerData); System.out.println("Exporting done.");
		 */
